...
Stitch is a **lightweight harness**: Cursor/Claude skills, contract/ledger templates, a SHA-256 fingerprint script, and a **gate CLI** that fails CI on drifted contracts or empty test evidence when `require_tests` is on. Orchestrator owns the contract; specialists implement one side; both sides must hash-match before the next phase.
...
- `stitch-orchestrator` / `stitch-client` / `stitch-server` skills  
- Contract + result + ledger templates  
- `check-fingerprint.sh` + `gate.sh` + `init-project.sh`  
...
Clone → `init-project.sh` → say “Stitch this endpoint.”
