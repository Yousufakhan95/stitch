...
> Map ledger ids → files on each side. Keep in sync with `ledgers:` in `sitch.yaml`.
> See docs/ledgers.md in the Sitch kit.
...
2. Add a row here and a `ledgers:` entry in `sitch.yaml`.
